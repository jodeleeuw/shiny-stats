...
getCor <- function(valA, valB, order){
  # dataA <- valA[order]
  # dataB <- valB[order]
  if(sd(valA[order])==0 | sd(valB[order])==0){return(NA)}
  c <- round(cor(valA[order], valB[order]),2)
  return(c)
}
...
shinyServer(function(input, output) {
  ...
  lastResampleDiff <- reactive({
    ...
    diff <- round(cor(ValuesA[NewGroup], ValuesB[NewGroup]),digits=2)
    return(diff)
  })
  ...
  resample <- function(n){
    ...
    dataA <- groupArv()
    dataB <- groupBrv()
    orders <- replicate(n, sample(1:length(dataB), replace = TRUE))
    rv$newOrder <- orders[,n]
    m <- apply(orders, MARGIN = 2, FUN = getCor, valA = dataA, valB = dataB )
    rv$outcomes <- c(rv$outcomes, m[!is.na(m)])
  }
  ...
  output$distPlot <- renderPlot({
    ...
    limMax <- max(sd(values)/(sd(values)+1)+ cor(dataA, dataB), max((rv$outcomes)))+.5
    limMin <- min(-sd(values)/(sd(values)+1)+ cor(dataA, dataB), min((rv$outcomes)))-.5
    if(length(rv$outcomes)>10){
      limMax <- max((rv$outcomes))+.2
      limMin <- min((rv$outcomes))-.2
    }
    
    p <- ggplot(freqtable, aes(x=val,y=freq, fill = inrange)) +
      geom_bar(stat="identity")+
      labs(y="Frequency\n",x="\nCorrelations in bootstrapped data")+
  ...
  output$lastResampleDiff <- renderText({
    ...
    return(paste0("The bootstrapped correlation ", diff))
  })
  
  output$resampledData <- renderPlot({
    As <- groupArv()
    Bs <- groupBrv()
    newOrder <- rv$newOrder
    ...
    # data frame for the plot
    def <- data.frame(A = matV[newOrder,1], B = matV[newOrder,2])
  ...
  output$evaluationPanel <- renderUI({
    if(is.null(rv$outcomes)){
      ...
      maxV <- (max(rv$outcomes))
      minV <- (min(rv$outcomes))
    }
